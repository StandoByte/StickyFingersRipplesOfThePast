...
import com.hk47bot.rotp_stfn.capability.*;
        ...
        if (!world.isClientSide() && user instanceof ServerPlayerEntity){
            ...
            switch (target.getType()) {
                case ENTITY:
                    ...
                    target.getEntity().getCapability(EntityZipperCapabilityProvider.CAPABILITY).ifPresent(zipperCap -> {
                        EntityZipperStorage storage = zipperCap.getOrCreateZipperStorage(target.getEntity());
                        NetworkHooks.openGui((ServerPlayerEntity) user, storage);
                    });
                    break;
                case BLOCK:
                    BlockPos pos = target.getBlockPos();
                    if (world.getBlockState(pos).getBlock() instanceof StickyFingersZipperBlock){
