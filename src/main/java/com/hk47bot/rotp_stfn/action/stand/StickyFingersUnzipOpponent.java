...
public class StickyFingersUnzipOpponent extends StandEntityHeavyAttack {

    public StickyFingersUnzipOpponent(Builder builder) {
        super(builder);
    }
    ...
    public void onTaskSet(World world, StandEntity standEntity, IStandPower standPower, Phase phase, StandEntityTask task, int ticks) {
        ...
        if (!world.isClientSide() && task.getTarget().getType() == ActionTarget.TargetType.ENTITY) {
    ...
    public void rotateStandTowardsTarget(StandEntity standEntity, ActionTarget target, StandEntityTask task) {
        if (task.getTarget().getType() == ActionTarget.TargetType.ENTITY && !task.getAdditionalData().isEmpty(TargetHitPart.class)) {
