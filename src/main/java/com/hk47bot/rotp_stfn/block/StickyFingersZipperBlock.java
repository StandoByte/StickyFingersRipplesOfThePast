...
//        }
    ...
    public static void placeSingleZipper(World world, BlockPos targetedBlockPos, Direction direction, LivingEntity placer){
        if (!world.isClientSide()) {
            ...
            BlockState prevBlockState = world.getBlockState(zipperPos1);
            LootContext.Builder lootcontext$builder = (new LootContext.Builder((ServerWorld)world)).withRandom(world.random).withParameter(LootParameters.ORIGIN, Vector3d.atCenterOf(zipperPos1)).withParameter(LootParameters.TOOL, ItemStack.EMPTY);
            prevBlockState.getDrops(lootcontext$builder);
    ...
    public BlockState updateShape(BlockState state, Direction direction, BlockState neighborState, IWorld world, BlockPos pos, BlockPos neighborPos) {
        ...
        BlockPos linkedZipperPos = StickyFingersZipperBlock.getLinkedZipperBlockPos(state, pos, world);
    ...
    public void setPlacedBy(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        ...
        setOwnerData(world, pos, placer);
    }

    private static void setOwnerData(World world, BlockPos pos, LivingEntity placer) {
        if (!world.isClientSide() && placer != null) {
            TileEntity tileEntity = world.getBlockEntity(pos);
            if (tileEntity instanceof StickyFingersZipperTileEntity) {
                ((StickyFingersZipperTileEntity) tileEntity).setOwner(placer.getUUID());
            }
        }
    }
}
